...
DOCKER_TAG?=jaytwo_http_authentication_oauth10a
...
build: restore
	dotnet build ./jaytwo.Http.Authentication.OAuth10a.sln
...
unit-test: build
	...
	rm -rf out/coverage
	cd ./test/jaytwo.Http.Authentication.OAuth10a.Tests; \
		dotnet test \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.Http.Authentication.OAuth10a.Tests.trx"
	reportgenerator \
...
pack:
	rm -rf out/packed
	cd ./src/jaytwo.Http.Authentication.OAuth10a; \
